...
df.head()
df['Dur']=df['Dur']*1000
drop_columns = ['Dir', 'Flgs','Attack Category','Pulse_Rate','Heart_rate','Resp_Rate']
...
drop_columns = ['SrcAddr',	'DstAddr', 'Dport',	'SrcBytes',	'DstBytes',	'SrcGap',	'DstGap',	'SIntPktAct',	'DIntPktAct',
                ...
                'pSrcLoss',	'pDstLoss',	'Rate',		'SpO2',	'SYS',	'DIA','ST']
...
from sklearn.model_selection import train_test_split
from sklearn.preprocessing import StandardScaler
for i in range(1,clientscount+1):
 X_train, X_test, y_train, y_test = train_test_split(X_smote, Y_smote, test_size=0.2, random_state = 42)
 scaling = StandardScaler()
 X_train = scaling.fit_transform(X_train)
 X_test = scaling.transform(X_test)
 ...
 model = tf.keras.models.Sequential([
        ...
        tf.keras.layers.LSTM(64, input_shape=(None, 12), return_sequences=False), 
 ...
 """
 model = tf.keras.models.Sequential()
 model.add(Convolution1D(64, 3, activation="relu",input_shape=(11,1 ,)))
...
#model.add(Dense(2, activation='softmax'))
 ...
 history = model.fit(X_train, y_train, epochs=epochcount,batch_size=512, validation_data=(X_test, y_test), verbose=1)#, class_weight=class_weights)
 ...
 hist.tail()
 tensorflow_federated_learning_average_accuracyscore=tensorflow_federated_learning_average_accuracyscore+history.history['val_accuracy'][epochcount-1]

tensorflow_federated_learning_average_accuracyscore= tensorflow_federated_learning_average_accuracyscore/clientscount
print('TensorFlow Federated Learning Accuracy:' , str(tensorflow_federated_learning_average_accuracyscore))
...
history.history['val_accuracy'][epochcount-1]
end_time = time.time()
...
print(f"Model execution time: {duration} seconds")
exit()
"""
from keras.layers import LSTM, SimpleRNN, GRU
from keras.layers import Dense, Dropout, Activation, Embedding
...
modelLSTM = tf.keras.models.Sequential([
        ...
        tf.keras.layers.LSTM(64, input_shape=(None, 34), return_sequences=False), 
        tf.keras.layers.Dense(32, activation='relu'),
        tf.keras.layers.Dense(64, activation='relu'),
        tf.keras.layers.Dense(32, activation='relu'),
...
modelLSTM.compile(loss='categorical_crossentropy',optimizer=Adam(lr=0.001),metrics=['accuracy'])

history = modelLSTM.fit(X_uni_train, y_uni_train, epochs=5,batch_size=512, validation_data=(X_uni_test, y_uni_test), verbose=1)#, class_weight=class_weights)

print(modelLSTM.evaluate(X_uni_test, y_uni_test))

y_preds_dnn = modelLSTM.predict(X_uni_test)
...
print('Summary of the LSTM results after each epoch')
hist = pd.DataFrame(history.history)
hist['epoch'] = history.epoch
hist.tail()
...
#!pip install --upgrade jaxlib==0.4.10
#!pip uninstall jax
#!pip install --upgrade -q git+https://github.com/google/jax
import tensorflow as tf
print("TensorFlow version:", tf.__version__)

#!pip install --upgrade tensorflow-federated==0.86.0
...
"""
trainer = tensorflow_federated.python.learning.build_federated_averaging_process(
    model_fn,
